...
func TestBuildTokenQuery_AllFields(t *testing.T) {
	...
	expected := "(token (3:acr 21:urn:siros:acr:passkey) (3:aud 11:backend-api) (3:sub 6:user-1) (3:tac 2:rw) (9:tenant_id 9:tenant-42))"
	if q != expected {
...
func TestBuildTokenQuery_MinimalFields(t *testing.T) {
	...
	expected := "(token (3:aud 3:api) (3:sub 6:user-1) (3:tac 1:r))"
	if q != expected {
		t.Errorf("unexpected query:\n  got:  %s\n  want: %s", q, expected)
	}
}
...
func TestBuildTokenQuery_Empty(t *testing.T) {
	q := BuildTokenQuery("", "", "", "", "")
	if q != "(token)" {
		t.Errorf("expected (token), got %s", q)
	}
}
