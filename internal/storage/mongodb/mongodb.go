...
func (s *UserStore) GetByID(ctx context.Context, id domain.UserID) (*domain.User, error) {
	...
	// string. The key here ("_id.id") is a fixed literal; id.String() is only ever
	// used as a plain field VALUE, which the driver BSON-encodes as a string and
	// compares by equality. A string value can never be interpreted as a Mongo
	// query operator (only "$"-prefixed map KEYS are), so untrusted input reaching
	// this call site cannot inject query semantics.
