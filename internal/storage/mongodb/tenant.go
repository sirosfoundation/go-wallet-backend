...
func (s *UserTenantStore) IsMember(ctx context.Context, userID domain.UserID, tenantID domain.TenantID) (bool, error) {
	// codeql[go/sql-injection] -- bson.M is a typed document builder, not a query
	// string. Both map keys ("user_id", "tenant_id") are fixed literals; userID
	// and tenantID are only ever used as plain field VALUES, BSON-encoded as
	// strings and compared by equality. A string value can never be interpreted
	// as a Mongo query operator (only "$"-prefixed map KEYS are), so untrusted
	// input reaching this call site cannot inject query semantics.
